git clone https://gitee.com/mirrors_sourceware_org/git_binutils-gdb.git readelf-readelf.c:8030
cd readelf-readelf.c:8030
git checkout 7fa3933
...
# echo 'readelf.c:2546' > $TMP_DIR/target.txt
# echo 'readelf.c:2012' > $TMP_DIR/target.txt
# echo 'readelf.c:3555' > $TMP_DIR/target.txt
echo 'readelf.c:8030' > $TMP_DIR/target.txt
# echo 'readelf.c:8031' > $TMP_DIR/target.txt
# echo 'readelf.c:12288' > $TMP_DIR/target.txt
# echo 'readelf.c:15892' > $TMP_DIR/target.txt
# echo 'readelf.c:15065' > $TMP_DIR/target.txt

$SVF_TOOL -target=$TMP_DIR/target.txt -cdbb-output=$TMP_DIR/cdbb.txt -ddbb-output=$TMP_DIR/ddbb.txt -dist-output=$TMP_DIR/dist.txt -deep-output=$TMP_DIR/deep.txt -cbrset-output=$TMP_DIR/cbrset.txt -brs-output=$TMP_DIR/brs.txt ./readelf.bc
...
# ~/aflgo/afl-2.57b/afl-fuzz -S fuzz8030 -m none -z exp -c 45m -i in -o out -- ./obj-aflgo-change/binutils/readelf -a @@
# ~/.cargo/bin/symcc_fuzzing_helper -o out -a fuzz8030 -n symcc -- ./obj-symcc/binutils/readelf -a @@