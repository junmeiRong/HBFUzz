...
# echo 'archive64.c:73' > $TMP_DIR/target.txt
# echo 'ihex.c:373' > $TMP_DIR/target.txt
# echo 'hash.c:350' > $TMP_DIR/target.txt
# echo 'elfcode.h:729' > $TMP_DIR/target.txt
echo 'elf.c:439' > $TMP_DIR/target.txt
...
# ~/aflgo/afl-2.57b/afl-fuzz -S fuzz439 -m none -z exp -c 45m -i in -o out -- ./obj-aflgo-change/binutils/nm-new -A -a -l -S -s --special-syms --synthetic --with-symbol-versions -D @@
# ~/.cargo/bin/symcc_fuzzing_helper -o out -a fuzz439 -n symcc -- ./obj-symcc/binutils/nm-new -A -a -l -S -s --special-syms --synthetic --with-symbol-versions -D @@